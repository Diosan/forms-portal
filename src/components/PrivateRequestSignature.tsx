...
export const PrivateRequestSignature = ({ submission_id, complainant_email }: RequestSignatureProps) => {
            ...
            if(!indictment) {
                ...
                try {
                    ...
                    if (requestResult.data.outcome == 'success') {
                        navigate('/private_view/' + submission_id)
                    } else {
    ...
    useEffect(() => {
        ...
        const getForm = async () => {
            let summaryForm = await axios.get(API_URL + '/schema/private_summary')
